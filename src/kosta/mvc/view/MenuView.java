...
public class MenuView {
	...
	public static void printUserMenu(String userId) {
		while(true) {
			SessionSet ss = SessionSet.getInstance();
			System.out.println(ss.getSet()); //Set 객체 (로그인된 사용자 출력)
	...
	public static void logout(String userId) {
		Session session = new Session(userId); // 인수로 들고온 ID로 새로운 Session객체 생성 
		SessionSet ss = SessionSet.getInstance(); // SessionSet 객체를 얻어옴 
		ss.remove(session);	 // SessionSet의 remove 메소드를 통해 생성한 Session객체 삭제 .. 
	}
    ...
    public static void printInputOrder(String userId) {
    	System.out.print("주문상품번호 : ");
		 ...
		 String userid=null;
		 for(Session session : sessionSet.getSet()) {
